...
@inline function accumulate!(config, annotation, tangent, batch)
  ...
  target =
    EnzymeRules.width(config) == 1 ? annotation.dval : annotation.dval[batch]
...
@inline active_tangent(a::EnzymeCore.Active{T}, tangent) where {T} =
  convert(T, tangent)::T
...
@inline _cell(::Type{Any}, result) =
  ismutable(result) ? zero(result) : Ref(zero(result))
...
function EnzymeRules.augmented_primal(
  ...
  return EnzymeRules.augmented_rule_return_type(config, RT){typeof(tape)}(
    EnzymeRules.needs_primal(config) ? result : nothing,
    shadow,
    tape
  )
end
...
function EnzymeRules.reverse(
  ...
  arg_tangent =
    arg isa EnzymeCore.Active ?
    (width == 1 ? tangents[1][1] : ntuple(i -> tangents[i][1], Val(width))) :
    nothing
  params_tangent =
    params isa EnzymeCore.Active ?
