...
const ENZ_LOSSY_B = SimpleChains.add_loss(
    ENZ_CHAIN, SquaredLoss(Float32[0.5 -0.2 0.1 0.4; -0.5 0.3 -0.1 0.2])
)
...
const ENZ_WIDE_P = Array(SimpleChains.init_params(ENZ_WIDE; rng = MersenneTwister(5)))
...
@testset "Enzyme" begin
    ...
    @testset "batched parameters with an active input" begin
        ...
        result = Enzyme.autodiff(
            Enzyme.Reverse, enz_vjpdot, Enzyme.Active, Enzyme.Const(ENZ_CHAIN),
            Enzyme.Active(us), Enzyme.BatchDuplicated(ENZ_P, (da, db)), Enzyme.Const(λ)
        )
        ref_dot_u = ForwardDiff.gradient(x -> enz_vjpdot(ENZ_CHAIN, x, ENZ_P, λ), ENZ_U)
    ...
    @testset "loss terminated chain with a static array input" begin
        ...
        result = Enzyme.autodiff(
            Enzyme.Reverse, enz_call, Enzyme.Active, Enzyme.Const(ENZ_LOSSY),
            Enzyme.Active(us), Enzyme.Duplicated(ENZ_P, dp)
        )
    ...
    @testset "MVector input on a chain without a loss" begin
        ...
        @testset "constant input" begin
            ...
            Enzyme.autodiff(
                Enzyme.Reverse, enz_sumsq, Enzyme.Active, Enzyme.Const(ENZ_CHAIN),
                Enzyme.Const(um), Enzyme.Duplicated(ENZ_P, dp)
            )
            @test dp ≈ ref_p
        end
        @testset "active input" begin
            ...
            Enzyme.autodiff(
                Enzyme.Reverse, enz_sumsq, Enzyme.Active, Enzyme.Const(ENZ_CHAIN),
                Enzyme.Duplicated(um, du), Enzyme.Duplicated(ENZ_P, dp)
            )
            @test du ≈ ref_u
            @test dp ≈ ref_p
        end
    ...
    @testset "loss terminated chain with a non-unit seed" begin
        ...
        Enzyme.autodiff(
            Enzyme.Reverse, enz_scaled, Enzyme.Active, Enzyme.Const(ENZ_LOSSY),
            Enzyme.Duplicated(ENZ_U, du), Enzyme.Duplicated(ENZ_P, dp)
        )
    ...
    @testset "batched input on a loss terminated chain" begin
        ...
        Enzyme.autodiff(
            Enzyme.Reverse, enz_call, Enzyme.Active, Enzyme.Const(ENZ_LOSSY_B),
            Enzyme.Duplicated(ENZ_U_B, dU), Enzyme.Duplicated(ENZ_P, dp)
        )
    ...
    @testset "inactively used result" begin
        @testset "result completely unused" begin
            dp = zero(ENZ_P)
            Enzyme.autodiff(
                Enzyme.Reverse, enz_inactive_1, Enzyme.Active, Enzyme.Const(ENZ_CHAIN),
                Enzyme.Const(ENZ_U), Enzyme.Duplicated(ENZ_P, dp)
            )
            @test dp ≈ ForwardDiff.gradient(q -> enz_inactive_1(ENZ_CHAIN, ENZ_U, q), ENZ_P)
        end
        @testset "result used in a non-differentiable way" begin
            dp = zero(ENZ_P)
            Enzyme.autodiff(
                Enzyme.Reverse, enz_inactive_2, Enzyme.Active, Enzyme.Const(ENZ_LOSSY),
                Enzyme.Const(ENZ_U), Enzyme.Duplicated(ENZ_P, dp)
            )
            @test dp ≈
    ...
    @testset "the chain itself must be Const" begin
        ...
        @test_throws "chain itself to be `Enzyme.Const`" Enzyme.autodiff(
            Enzyme.Reverse, enz_call, Enzyme.Active,
            Enzyme.Duplicated(ENZ_LOSSY, dchain),
            Enzyme.Const(ENZ_U), Enzyme.Duplicated(ENZ_P, zero(ENZ_P))
        )
    end
    ...
    @testset "SVector parameters are rejected" begin
        ...
        @test_throws "pointer-backed" Enzyme.autodiff(
            Enzyme.Reverse, enz_sumsq, Enzyme.Active, Enzyme.Const(ENZ_CHAIN),
            Enzyme.Const(ENZ_U), Enzyme.Active(ps)
        )
        @test_throws "pointer-backed" Enzyme.autodiff(
            Enzyme.Reverse, enz_call, Enzyme.Active, Enzyme.Const(ENZ_LOSSY),
            Enzyme.Duplicated(ENZ_U, zero(ENZ_U)), Enzyme.Active(ps)
        )
        # The parameter values are needed for the input gradient too,
        # so freezing them does not help
        @test_throws "pointer-backed" Enzyme.autodiff(
            Enzyme.Reverse, enz_sumsq, Enzyme.Active, Enzyme.Const(ENZ_CHAIN),
            Enzyme.Duplicated(ENZ_U, zero(ENZ_U)), Enzyme.Const(ps)
        )
        ...
        forward, _reverse = Enzyme.autodiff_thunk(
            Enzyme.ReverseSplitWithPrimal, Enzyme.Const{typeof(enz_call)}, Enzyme.Active,
            Enzyme.Const{typeof(ENZ_LOSSY)}, Enzyme.Const{typeof(ENZ_U)},
            Enzyme.Active{typeof(ps)}
        )
        @test_throws "pointer-backed" forward(
            Enzyme.Const(enz_call), Enzyme.Const(ENZ_LOSSY), Enzyme.Const(ENZ_U),
            Enzyme.Active(ps)
        )
